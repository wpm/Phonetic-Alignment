...
module PhoneticAlign
    ...
    def each_morpheme_hypothesis
      ...
      if different_segments.length == 1
        ...
        if different_segment.is_phones?(:source)
          ...
          yield DifferenceHypothesis.new(different_segment, :source, morph)
        end
        if different_segment.is_phones?(:dest)
          ...
          yield DifferenceHypothesis.new(different_segment, :dest, morph)
        end
      end
      ...
      if same_segments.length == 1
        ...
        if segment.is_phones?(:source) # If source is phones so is dest.
          ...
          yield SimilarityHypothesis.new(same_segment, :source, source_morph)
          dest_morph = SurfaceMorpheme.new(shared_meaning, dest_allomorph,
                                          [source_allomorph, dest_allomorph])
          yield SimilarityHypothesis.new(same_segment, :dest, dest_morph)
        end
      end
      ...
      [:source, :dest].each do |word|
        ...
        if phone_sequences.length == 1
          ...
          if not meaning.empty?
            ...
            yield FromWordHypothesis.new(phone_sequences.first, word, morph)
          end
        end
      end
    end
  ...
  # A morpheme hypothesis generated from a phonetically different segment.
  class DifferenceHypothesis < MorphemeHypothesis
  end


  # A morpheme hypothesis generated from a phonetically same segment.
  class SimilarityHypothesis < MorphemeHypothesis
  end


  # A morpheme hypothesis generated by comparing a phonetic segment with an
  # entire word's meaning.
  class FromWordHypothesis < MorphemeHypothesis
  end
